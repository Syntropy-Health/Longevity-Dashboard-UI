...
def checkin_card(checkin: dict) -> rx.Component:
    """Check-in card with optional transcript view toggle."""
    type_icons = {
        "voice": "mic",
        "text": "message-square",
        "call": "phone",
    ...
    return rx.el.div(
        ...
        rx.el.div(
            # Transcript toggle button (only for call type)
            rx.cond(
                checkin["type"] == "call",
                rx.el.button(
                    rx.icon("file-text", class_name="w-4 h-4 mr-1"),
                    "View Transcript",
                    on_click=lambda: CheckinState.open_transcript_modal(checkin),
                    class_name="px-2 py-1 rounded text-[10px] font-medium bg-purple-500/10 text-purple-300 border border-purple-500/20 hover:bg-purple-500/20 transition-colors flex items-center mr-2",
                ),
                rx.fragment(),
            ),
            rx.cond(
...
def checkins_tab() -> rx.Component:
    ...
    return rx.el.div(
        ...
        rx.el.div(
            ...
            rx.el.div(
                rx.foreach(CheckinState.checkins, checkin_card),
