...
def status_badge(status: str) -> rx.Component:
    ...
    colors = status_colors.get(status.lower() if isinstance(status, str) else "pending", status_colors["pending"])
...
def time_slot_button(time: str, available: bool, selected: bool, on_click) -> rx.Component:
    ...
    return rx.box(
        rx.text(time, class_name=f"text-sm {'text-white' if selected else 'text-slate-300'}"),
...
def appointment_card(appointment: dict, on_click=None) -> rx.Component:
    ...
    return rx.box(
        ...
        class_name=GlassStyles.CARD_INTERACTIVE + " cursor-pointer hover:bg-slate-700/60 transition-all duration-200",
        on_click=on_click,
    )
                    ...
                    lambda week: rx.hstack(
                        rx.foreach(
                            ...
                            lambda day: rx.cond(
                                day > 0,
                                rx.box(
                                    rx.text(day, class_name=rx.cond(
...
def time_slots_grid(state) -> rx.Component:
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.text("Afternoon", class_name="text-slate-400 text-xs font-medium mb-2 mt-4"),
            rx.hstack(
                rx.foreach(
                    afternoon_slots,
                    lambda t: time_slot_button(
