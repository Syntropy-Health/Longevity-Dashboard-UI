...
class AppConfig(BaseModel):
    ...
    vlogs_parse_with_llm: bool = True  # Enable LLM parsing by default
...
# Create agent with app config (parse_with_llm=True by default)
...
config = VlogsConfig(
    parse_with_llm=False,  # Disable LLM for faster processing
...
### Non-LLM Mode (`parse_with_llm=False`)
...
**Development**:
```python
config = VlogsConfig(parse_with_llm=False, limit=5)  # Fast iteration
```
...
- ✅ Set parse_with_llm=True as default
