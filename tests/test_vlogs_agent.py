...
class TestVlogsConfig:
    ...
    def test_default_config(self):
        ...
        config = VlogsConfig()
        assert config.parse_with_llm is True  # Default should be True
    ...
    def test_custom_config(self):
        ...
        assert config.parse_with_llm is False
    ...
    def test_default_config_from_app_settings(self):
        ...
        assert config.parse_with_llm is True
        assert config.llm_model == "gpt-4o-mini"
...
class TestVlogsAgent:
    ...
    def test_agent_from_config(self):
        ...
        assert agent.config.parse_with_llm is True

    def test_agent_with_custom_config(self):
        """Test agent with custom configuration."""
        config = VlogsConfig(parse_with_llm=False, limit=10)
    ...
    async def test_process_logs_with_llm(self):
        ...
        """
        # Enable LLM parsing
        config = VlogsConfig(parse_with_llm=True, limit=2)
        agent = VlogsAgent(config=config)
...
def test_vlogs_agent_manually():
    ...
    async def run_test():
        ...
        config = VlogsConfig(parse_with_llm=False, limit=3)
        agent = VlogsAgent(config=config)
        ...
        if outputs:
            print(f"   ✅ First output has {len(outputs[0].medications_entries)} medications")
